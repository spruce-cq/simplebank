...
func TestTransfer(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		index := int(diff1 / amount)
		require.True(t, index >= 1 && index <= n)
		require.NotContains(t, exists, index)
		exists[index] = true
	}
	...
	var totalAmount = int32(n) * amount
...
func TestTransferWithDeadLock(t *testing.T) {
	store := NewStore(testDB)
